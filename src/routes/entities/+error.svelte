<!--
  @component Entities Error Page

  Issue #508: Add error boundaries and custom error page

  Displays an entity-themed error page for errors in entity routes.
-->
<script lang="ts">
	import { page } from '$app/stores';
	import { Users, Home, ArrowLeft, RefreshCw, ChevronDown, ChevronUp } from 'lucide-svelte';
	...
	const title = $derived.by(() => {
		...
		if (status === 500) return 'Entity Error';
		return 'Error';
	});

	const description = $derived.by(() => {
		if (status === 404) return "The entity you're looking for doesn't exist.";
		if (status === 500) return 'Something went wrong while loading this entity.';
		return 'An error occurred in the entities section.';
	});
		...
		<div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-8">
			...
			<div class="flex justify-center mb-6">
				<div class="w-16 h-16 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center">
					<Users class="w-8 h-8 text-red-600 dark:text-red-400" aria-hidden="true" />
				</div>
			</div>
